...
void sumOfCiphertext(Ciphertext &res, Ciphertext &x, Scheme &scheme, SecretKey &secretKey, Parameters params) {
    res.copy(x);
    //perform sum of all elements in Prod
    for (long j = params.m-1; j > 0; j--){
        scheme.leftRotateFastAndEqual(x, 1);
        scheme.addAndEqual(res, x);
    }
}

void sumOfCiphertextVector(Ciphertext &res, vector<Ciphertext> &x, Scheme &scheme, SecretKey &secretKey, Parameters params) {
    complex<double>* temp = new complex<double>[params.m];
    for (int i = 0; i < params.m; i++){
        temp[i] = complex<double>(0.0, 0.0);
    }
    scheme.encrypt(res, temp, params.m, params.logp, params.logq);
    if (res.logq > x[0].logq) {
        scheme.modDownByAndEqual(res, res.logq - x[0].logq);
        for (int i = 0; i < x.size(); i++){
            scheme.addAndEqual(res, x[i]);
        }
    }
    else if (res.logq < x[0].logq) {
        for (int i = 0; i < x.size(); i++){
            scheme.modDownByAndEqual(x[i], x[i].logq - res.logq);
            scheme.addAndEqual(res, x[i]);
        }
    }
}
...
void inv_root(Ciphertext &result, Ciphertext &x, Scheme &scheme, SecretKey &secretKey, Parameters params) {
    ...
    // Ciphertext result;
    scheme.encrypt(result, temp, params.m, params.logp, params.logq);
...
void dotProduct(Ciphertext &res, Ciphertext &x, Ciphertext &y, Scheme &scheme, SecretKey &secretKey, Parameters params){
    ...
    // Sum of elements
    sumOfCiphertext(res, prod, scheme, secretKey, params);
}
...
int main() {
    ...
    Parameters params = {logp, logq, n, m};
    // int ref = 8;
    ...
    for(int ref = 0; ref < embeddings.size(); ref++){
        for (int query = ref; query < embeddings.size(); query++) {
            ...
            for (int i = 0; i < reference_P.size(); i++){
                Ciphertext temp;
                scheme.mult(temp, reference_P[i], query_P[i]);
                scheme.reScaleByAndEqual(temp, temp.logp - logp);
                numerator.push_back(temp);
                temp.free();
            }

            Ciphertext numerator_sum;
            sumOfCiphertextVector(numerator_sum, numerator, scheme, secretKey, params);

            complex<double>* zero = new complex<double>[m];
            for (int i = 0; i < m; i++){
                zero[i] = complex<double>(0.0, 0.0);
            }
            complex<double>* check_p = scheme.decrypt(secretKey, numerator_sum);
            StringUtils::compare(zero, check_p, 2, "Numerator sum");

            vector<Ciphertext> reference_P_sq, query_P_sq;
            for (int i = 0; i < reference_P.size(); i++){
                Ciphertext temp, temp1;
                algo.powerOf2(temp, reference_P[i], logp, 1);
                algo.powerOf2(temp1, query_P[i], logp, 1);
                reference_P_sq.push_back(temp);
                query_P_sq.push_back(temp1);
                temp.free();
                temp1.free();
            }

            Ciphertext query_P_sq_sum, reference_P_sq_sum;
            sumOfCiphertextVector(query_P_sq_sum, query_P_sq, scheme, secretKey, params);
            sumOfCiphertextVector(reference_P_sq_sum, reference_P_sq, scheme, secretKey, params);

            check_p = scheme.decrypt(secretKey, query_P_sq_sum);
            StringUtils::compare(zero, check_p, 2, "query P sum");

            check_p = scheme.decrypt(secretKey, reference_P_sq_sum);
            StringUtils::compare(zero, check_p, 2, "ref P sum");

            scheme.multByConstAndEqual(numerator_sum, 1.0/1024.0, logp);
            scheme.reScaleByAndEqual(numerator_sum, numerator_sum.logp - logp);

            scheme.multByConstAndEqual(query_P_sq_sum, 1.0/1024.0, logp);
            scheme.reScaleByAndEqual(query_P_sq_sum, query_P_sq_sum.logp - logp);

            scheme.multByConstAndEqual(reference_P_sq_sum, 1.0/1024.0, logp);
            scheme.reScaleByAndEqual(reference_P_sq_sum, reference_P_sq_sum.logp - logp);

            Ciphertext inv_root_query, inv_root_reference;
            inv_root(inv_root_query, query_P_sq_sum, scheme, secretKey, params);
            inv_root(inv_root_reference, reference_P_sq_sum, scheme, secretKey, params);

            check_p = scheme.decrypt(secretKey, inv_root_query);
            StringUtils::compare(zero, check_p, 2, "inv query P sum");

            check_p = scheme.decrypt(secretKey, inv_root_reference);
            StringUtils::compare(zero, check_p, 2, "inv ref P sum");

            if(numerator_sum.logp > logp){
                scheme.reScaleByAndEqual(numerator_sum, numerator_sum.logp - logp);
            }

            if(numerator_sum.logq > inv_root_query.logq) {
                scheme.modDownByAndEqual(numerator_sum, numerator_sum.logq - inv_root_query.logq);
            }
            else if(numerator_sum.logq < inv_root_query.logq) {
                scheme.modDownByAndEqual(inv_root_query, inv_root_query.logq - numerator_sum.logq);
            }
            // cout << inv_root_x.logp << " " << inv_root_x.logq << endl;
            // cout << inv_root_y.logp << " " << inv_root_y.logq << endl;
            // cout << num_sum.logp << " " << num_sum.logq << endl;
            scheme.multAndEqual(numerator_sum, inv_root_query);
            scheme.reScaleByAndEqual(numerator_sum, numerator_sum.logp - logp);
            check_p = scheme.decrypt(secretKey, numerator_sum);
            StringUtils::compare(zero, check_p, 2, "Numerator sum 1st mul");

            if(numerator_sum.logq > inv_root_reference.logq) {
                scheme.modDownByAndEqual(numerator_sum, numerator_sum.logq - inv_root_reference.logq);
            }
            else if(numerator_sum.logq < inv_root_reference.logq) {
                scheme.modDownByAndEqual(inv_root_reference, inv_root_reference.logq - numerator_sum.logq);
            }

            scheme.multAndEqual(numerator_sum, inv_root_reference);
            scheme.reScaleByAndEqual(numerator_sum, numerator_sum.logp - logp);
            check_p = scheme.decrypt(secretKey, numerator_sum);
            StringUtils::compare(zero, check_p, 2, "Numerator sum 2nd mul");
                    
            // Computing true value

            complex<double>* true_val = new complex<double>[4];
            double true_val_num = 0.0, true_val_query = 0.0, true_val_ref = 0.0;
            for (int i = 0; i < n; i += m) {
                ...
                true_val_num += true_reference_P*true_query_P;
                true_val_query += pow(true_query_P, 2);
                true_val_ref += pow(true_reference_P, 2);
            }
            double true_val_temp = true_val_num / (sqrt(true_val_query) * sqrt(true_val_ref));
            ...
            complex<double>* decrypt_p = scheme.decrypt(secretKey, numerator_sum);

            StringUtils::compare(true_val, decrypt_p, 2, "Cosine");
