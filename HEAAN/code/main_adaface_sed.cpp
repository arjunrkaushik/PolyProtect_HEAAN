...
int main() {
    ...
    for (int i = 0; i < embeddings.size(); i++) {
        ...
        double epsilon = 0.1;
        double scale = 1.0 / (sensitivity / epsilon);

        random_device rd;
        mt19937_64 gen(rd());

        exponential_distribution<double> exponential(scale);

        double e1 = exponential(gen);
        double e2 = exponential(gen);
        double noise  = e1 - e2;
        cout << noise << endl;
        cout << "Start" << endl;
        while ( abs(noise) >= 0.001) {
            noise = noise / 10;
        }
        cout << noise << endl;

        for (int j = 0; j < embeddings[i].size(); j++) {
            embeddings[i][j] += noise;
        }
    }
